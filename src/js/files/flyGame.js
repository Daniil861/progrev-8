import { addMoney, deleteMoney, getRandom } from '../files/functions.js';
// game
...
const player = document.querySelector('.fly-game__player');
const info = document.querySelector('.fly-game__info');
const gameCount = document.querySelector('.fly-game__count-box');
...
function getPath() {
	...
	gameConfig.lengthPath = startCoord - topCoord;
...
function translatePercentToPath(percent) {
...
function animateFlyGame() {
	...
	if (gameConfig.yOffset < gameConfig.randomNumber) {
		...
		const winCount = Math.floor(+localStorage.getItem('current-bet') * gameConfig.yOffset / 10);
